strategy_1 <- function(trading_dates, rates, fx_spot, PL_d, PL_p, X_out)
{
  ...
  for(i in 1:length(trading_dates)) #move along trading dates, then adjust hours; could have done the opposite
  {
    ...
    for(j in 1:(nrow(fx_spot_i)))
    {
      ...
      if(i >= 2 & (abs(fx_spot_i$Spot[j] - Target) <= 0.001 || (i == length(trading_dates) & j == nrow(fx_spot_i) ) ) )
      {
        ...
        print(paste("c_last_close_if_rolling_took_place = ", c_close_if_rolling_took_place, sep = ""))
        print(paste("c_last_open_if_rolling_took_place = ", c_open_if_rolling_took_place, sep = ""))
        print(paste("number_of_rolling_times = ", number_of_rolling_times, sep = ""))
        print(paste("PL_d_after_rolling = ", PL_d_after_rolling, sep = ""))
        
        print("                                                 ")
        print(paste("______________ TRADING RESULTS FOR GIVEN PERIOD: from ", trading_dates[1], " to ", trading_dates[length(trading_dates)], "________________", sep = ""))
